...
def RepostMain(accountlist,url_to_craw,checkey,figure,proxylist,startTime,pauseTime):
	...
	cursor.execute('create table '+filename+' (id CHAR(12),name CHAR(50),time DATETIME, content TEXT,repofrom CHAR(50),polvl SMALLINT) ENGINE=MyISAM DEFAULT CHARSET=gbk')
    except Exception,e:
    ...
    poid=WeiboMain.get_poid(htmlContent)
    #print 'poname=',poname,'\n'
    #poname=WeiboMain.character(poname)
    #print 'po'
    #print poname,'\n'
    cursor.execute("insert into %s (id,name,time,polvl) values ('%s','%s','%s',%s)" % (filename,poid,WeiboMain.character(poname),potime,0))
	...
	while pagepoint<pages:
		...
		WeiboMain.get_info(htmlContent,dataFile,cursor=cursor,filename=filename,poid=poid)
		conn.commit()
