...
def get_info(content,data_file,cursor,filename,poid):#get infomamtion according to content(source code) and put it into data_file
# 
    ...
    #print contents[5]
    pattern=re.compile(r'''<a href=\\"\\\/.*?\\" title=\\"(.*?)\\".*? usercard=\\"id=(.*?)\\".*?<\\\/a>.*?<em>(.*?)<\\\/em>[\s\S]*?\\" title=\\"([0-9]{4}\-[0-9]{2}\-[0-9]{2} [0-9]{2}:[0-9]{2})\\"''')
    for con in contents[::]:
        #con=contents[conindex]
        result=pattern.search(con)
        if result is not None:
            name=result.group(1)
            userid=result.group(2)
            contents=result.group(3)
            final_time=result.group(4)
